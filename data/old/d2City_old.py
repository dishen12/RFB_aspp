...
D2CITY_ROOT = "/nfs/project/d2_city/public_data/ssd/"
...
class d2CityDetection(data.Dataset):
    ...
    def __init__(self, root='/nfs/project/d2_city/public_data/ssd',
                 image_sets=['train'],
                 transform=None, target_transform=d2CityAnnotationTransform(),
                 dataset_name='d2City'):
        self.root = root
        ...
        self._annopath = osp.join('%s', 'ann', '%s.txt')
        self._imgpath = osp.join('%s', 'video2Frames', '%s.jpg')
        self.ids = list()
        for seg in image_sets:
            for line in open(osp.join(self.root, 'Main', seg + '.txt')):
