...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes,Rate=[6,3,2,1]):
        ...
        self.aspp_a_4 = Aspp_b_2(512,512,stride=1,scale=1,rate=Rate)
    ...
    def forward(self, x):
        ...
        #s = x
        sources.append(s)
        #print("extras is ",self.extras)
        # apply extra layers and cache source layer outputs
        #在extra的每一层之后，都会有relu
...
def add_extras_aspp(cfg, i, batch_norm=False,Rate=[6,3,2,1]):
    # Extra layers added to VGG for feature scaling
    print("the rate of extra is:",Rate)
    layers = []
    in_channels = i
    flag = False
    for k, v in enumerate(cfg):
        if in_channels != 'S':
            if v == 'S':  #[256, 'S', 512, 128, 'S', 256, 128, 256, 128, 256]
                layers += [Aspp_b_2(in_channels,cfg[k+1],stride=2,scale=1,rate=Rate)]
            else:
                layers += [nn.Conv2d(in_channels, v, kernel_size=(1, 3)[flag])]
            flag = not flag
        in_channels = v
