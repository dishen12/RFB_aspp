...
class SeAttention(nn.Module):
    ...
    def forward(self,x):
        #Squeeze
        b,c,_,_ = x.size()
        w = self.avg_pool(x) # 此处记得看下feature map的具体组成
        #print("the size of w is",w.size(),"channel is ",self.planes)
        w = F.relu(self.fc1(w))
        w = F.sigmoid(self.fc2(w)).view(b,c,1,1)
        # Excitation
        out = x*w.expand_as(x)
        return out
...
class Aspp_b_2_mid_concat_relu(nn.Module):
    ...
    def __init__(self,in_planes,out_planes,stride=1,scale=0.1,rate=[6,3,2,1]):
        ...
        self.SeAttention_inter = SeAttention(8*inter_planes,16)
        self.SeAttention_out = SeAttention(24*inter_planes,16)
        if(len(rate)==4):
    ...
    def forward(self,x):
        ...
        if(len(self.rate)==4):
            ...
            out1 = self.relu(out1)
            #out1 = self.SeAttention_inter(out1)
            
            out2 = torch.cat((x01,x11,x21,x31),1)
            out2 = self.relu(out2)
            #out2 = self.SeAttention_inter(out2)
            
            out3 = torch.cat((x02,x12,x22,x32),1)
            out3 = self.relu(out3)
            #out3 = self.SeAttention_inter(out3)
            
            out = torch.cat((out1,out2,out3),1)
            out = self.SeAttention_out(out)
